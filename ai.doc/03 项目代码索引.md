...
| 文件 | 说明 |
|------|------|
| `taskStore.svelte.ts` | 任务状态管理，使用 Svelte 5 runes。包含 `startTask`/`pauseTask`/`resumeTask`/`completeTask`/`updateTask` 等方法，支持 localStorage 持久化 |
...
| `task/CheckpointItem.svelte` | 单个检查点记录条目 |
